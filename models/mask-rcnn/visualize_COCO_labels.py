...
PATH_TO_ANNOT = "c:/Users/Rohan/Downloads/annotations_trainval2017/annotations/instances_train2017.json" # "c:/Users/Rohan/datasets/coco/annotations/instances_val2017.json"
        ...
        for ann in img_annotations:
            ...
            if display_type in ['seg', 'both']:
                for seg in ann['segmentation']:
                    poly = [(seg[i], seg[i+1]) for i in range(0, len(seg), 2)] # List of points
                    polygon = patches.Polygon(poly, closed=True, edgecolor=color, color=color, fill=True) # Remove color=color and set fill=False
...
# Get all image files
all_image_files = [os.path.join(PATH_TO_TRAIN, img['file_name']) for img in annotations['images']]
